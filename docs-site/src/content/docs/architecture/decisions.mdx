...
| AD-019 | Guided configuration is an offline browser wizard; no terminal UI is planned | The handoff contract is UI-neutral (`ui-selection`); the wizard holds no token, accepts no secret, and applies no policy |
...
- The repository contains an installable module that plans, authenticates,
  reconciles, deploys, probes, and tears down, plus documentation and backlog.
- Nothing has been verified against a live Azure subscription and no release
  has been published, so no behavior is **Available**.
...
Implemented in the repository, pending verification against a live
subscription and a published release:

- Versioned YAML schema and provenance resolver.
- Normalized plan canonicalization, validation, and hashing.
- Local event state, snapshots, locking, and reconciliation.
- The PowerShell engine and SQL VM provider end to end.
- Evidence renderers and HTML output.
- Governed catalogs, probes, cost, Key Vault, and Bastion capabilities.

Still outstanding:

- Extract the deployment engine behind a registered contract before a second
  engine or target provider lands.
- Remote state for unattended runs.
